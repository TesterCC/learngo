		...
		for {
			...
			case uploadBytes := <-uploadBytesCh:
				if uploadBytes >= totalUploadBytes {
					...
					// 77%整数百分比展示
					//fmt.Printf("上传进度: %d 字节，%d%%\n", uploadBytes, int64(math.Round(float64(uploadBytes)/float64(fileSize)*100)))
					// 77.77%浮点数百分比展示
					fmt.Printf("上传进度: %d 字节，%.2f%%\n", uploadBytes, float64(uploadBytes)/float64(fileSize)*100)
					// 最后100%会打印两次，但不影响检测，此问题已在uploadclient.go中修复
				}
			case <-done:
				return
			}
